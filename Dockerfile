...
COPY --from=package-builder /app/dist/mapio_display*.whl /app/
RUN pip --python "$(which python)" install --no-cache-dir --no-deps ./mapio_display*.whl

ENTRYPOINT [ "mapio_display" ]
